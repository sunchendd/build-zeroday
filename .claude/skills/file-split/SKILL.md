...
The split script is at `.opencode/skills/file-split/file-split.sh`.

Usage:
```bash
bash .opencode/skills/file-split/file-split.sh <file_path> [split_size]
```
...
1. Run the script with the file path:
   ```bash
   bash .opencode/skills/file-split/file-split.sh /path/to/large_file.tar
   ```
2. Or with a custom split size:
   ```bash
   bash .opencode/skills/file-split/file-split.sh /path/to/large_file.tar 5G
   ```
...
```bash
bash .opencode/skills/file-split/file-split.sh /os_nfs/06_images/Wings_vllm-ascend_v0.20.2rc1-a3_910C_Qwen36-27B_aarch64.tar
```
